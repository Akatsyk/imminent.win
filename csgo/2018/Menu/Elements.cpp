...
namespace Menu {
	...
	void DrawMenu() {
		if (GUI::Form::BeginWindow(XorStr("imminent.win")) || GUI::ctx->setup) {
			...
			if (GUI::Form::BeginTab(0, XorStr("A")) || GUI::ctx->setup) {
				...
				GUI::Controls::Label(XorStr("Distortion Resolver"));
				GUI::Controls::Hotkey(XorStr("##distoresolverkey"), & g_Vars.rage.distortion_resolver);
				...
				GUI::Controls::Checkbox(XorStr("Automatic fire"), &g_Vars.rage.auto_fire);
				{
					...
					if (rbot->active || GUI::ctx->setup) {
						...
						//if (GUI::Controls::Checkbox(XorStr("Static pointscale") + std::string(XorStr("#") + std::to_string(rage_current_group)), &rbot->static_point_scale) || GUI::ctx->setup) {
							GUI::Controls::Slider(XorStr("Point scale##687687675") + std::string(XorStr("#") + std::to_string(rage_current_group)), &rbot->point_scale, 0.f, 100.0f, rbot->point_scale <= 0.f ? XorStr("Dynamic") : XorStr("%.0f%%"));
							GUI::Controls::Slider(XorStr("Stomach scale##68776678") + std::string(XorStr("#") + std::to_string(rage_current_group)), &rbot->body_point_scale, 0.f, 100.0f, rbot->body_point_scale <= 0.f ? XorStr("Dynamic") : XorStr("%.0f%%"));
						//}

						if (GUI::Controls::Checkbox(XorStr("Hitchance") + std::string(XorStr("#") + std::to_string(rage_current_group)), &rbot->run_hitchance) || GUI::ctx->setup) {
							GUI::Controls::Slider(XorStr("Hitchance Amount") + std::string(XorStr("#") + std::to_string(rage_current_group)), &rbot->hitchance, 0.f, 100.f, XorStr("%.0f%%"));
						}

						if (GUI::Controls::Checkbox(XorStr("Accuracy boost") + std::string(XorStr("#") + std::to_string(rage_current_group)), &rbot->accuracy_boost) || GUI::ctx->setup) {
							GUI::Controls::Slider(XorStr("Accuracy Boost Amount") + std::string(XorStr("#") + std::to_string(rage_current_group)), &rbot->accuracy_boost_amount, 0.f, 100.0f, XorStr("%.0f%%"));
						}

						GUI::Controls::Slider(XorStr("Minimum Damage") + std::string(XorStr("#") + std::to_string(rage_current_group)), &rbot->min_damage_visible, 1.f, 130.f, rbot->min_damage_visible > 100 ? (std::string(XorStr("HP + ")).append(std::string(std::to_string(rbot->min_damage_visible - 100)))) : XorStr("%d hp"));

						if (GUI::Controls::Checkbox(XorStr("Autowall") + std::string(XorStr("#") + std::to_string(rage_current_group)), &rbot->autowall) || GUI::ctx->setup) {
							GUI::Controls::Slider(XorStr("Autowall Damage") + std::string(XorStr("#") + std::to_string(rage_current_group)), &rbot->min_damage, 1.f, 130.f, rbot->min_damage > 100 ? (std::string(XorStr("HP + ")).append(std::string(std::to_string(rbot->min_damage - 100)))) : XorStr("%d hp"));
						}
						...
						if (GUI::Controls::Checkbox(XorStr("Damage Override") + std::string(XorStr("#") + std::to_string(rage_current_group)), &rbot->min_damage_override) || GUI::ctx->setup) {
							GUI::Controls::Hotkey(XorStr("Damage Override Key#key") + std::string(XorStr("#") + std::to_string(rage_current_group)), &g_Vars.rage.key_dmg_override);
							GUI::Controls::Slider(XorStr("Override Amount##slider") + std::string(XorStr("#") + std::to_string(rage_current_group)), &rbot->min_damage_override_amount, 1.f, 130.f, rbot->min_damage_override_amount > 100 ? (std::string(XorStr("HP + ")).append(std::string(std::to_string(rbot->min_damage_override_amount - 100)))) : XorStr("%d hp"));
						}
					}
				}
				GUI::Group::EndGroup();
				...
				GUI::Group::BeginGroup(XorStr("Fake lag"), Vector2D(50, 40)); 
				{
					...
					GUI::Controls::Dropdown(XorStr(""), { XorStr("Maximum"), XorStr("Dynamic"), XorStr("Fluctuate") }, &g_Vars.fakelag.choke_type);
				...
				GUI::Group::BeginGroup(XorStr("Anti aim"), Vector2D(50, 50));
				{
					...
					if (settings->yaw > 0)
					{
						GUI::Controls::Dropdown(XorStr("Twist"), { XorStr("Off"), XorStr("Skylet"), XorStr("Proper") }, &settings->twist);

						if (settings->yaw == 3)
							GUI::Controls::Slider(XorStr(""), &g_Vars.antiaim.break_lby, -145, 145);
					}
					...
					switch (settings->base_yaw) {
					case 2:
						GUI::Controls::Slider(XorStr("##180jitterrange"), &settings->OneEightyJitterRange, 0, 180);
						break;
					case 3:
						GUI::Controls::Slider(XorStr("##spinrange"), &settings->RotationSpeed, -180, 180);
						GUI::Controls::Slider(XorStr("##spinspeed"), &settings->RotationRange, 0, 20);
						break;
					case 4:
						GUI::Controls::Slider(XorStr("##randomrange"), &settings->RandomRange, 0, 180);
						break;
					case 5:
						GUI::Controls::Slider(XorStr("##staticrange"), &settings->staticangle, 0, 180);
						break;
					}
					...
					case 2:
						GUI::Controls::Slider(XorStr("##2"), &settings->relativeamount, -180, 180);
						break;
					case 3:
						GUI::Controls::Slider(XorStr("range##3"), &settings->jitterrange, -180, 180);
						break;
			...
			if (GUI::Form::BeginTab(2, XorStr("C")) || GUI::ctx->setup) {
				...
				GUI::Group::BeginGroup(XorStr("Other ESP"), Vector2D(50, 50));
				{
					...
					GUI::Controls::Checkbox(XorStr("Indicators"), &g_Vars.esp.indicators);
			...
			if (GUI::Form::BeginTab(4, XorStr("D")) || GUI::ctx->setup) {
				...
				{
					GUI::Controls::Checkbox(XorStr("Bunny hop"), &g_Vars.misc.autojump);
					GUI::Controls::Checkbox(XorStr("Air strafe"), &g_Vars.misc.autostrafer);
					...
					GUI::Controls::Checkbox(XorStr("Jump at edge"), &g_Vars.misc.edgejump);
					...
					GUI::Controls::Checkbox(XorStr("Log misses"), &g_Vars.esp.event_resolver);
					...
					if (GUI::Controls::Checkbox(XorStr("Ping spike") + std::string(XorStr("#") + std::to_string(rage_current_group)), &g_Vars.misc.extended_backtrack)) {
						GUI::Controls::Hotkey(XorStr("Ping spike qswdecxvrftgbyhnujmiko,l") + std::string(XorStr("#") + std::to_string(rage_current_group)), &g_Vars.misc.extended_backtrack_key);
						GUI::Controls::Slider(XorStr("Ping spike##amt") + std::string(XorStr("#") + std::to_string(rage_current_group)), &g_Vars.misc.extended_backtrack_time, 0.f, 1.f, XorStr("%.2fs"), 0.1f);
					}
				...
				GUI::Group::BeginGroup(XorStr("Settings"), Vector2D(50, 40));
				{
				...
				GUI::Group::BeginGroup(XorStr("Other"), Vector2D(50, 60));
				{
