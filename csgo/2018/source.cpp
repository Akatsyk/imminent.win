...
void __fastcall LagComp_FrameUpdatePostEntityThink(void* thisptr)
{
	...
	for (int i = 0; i < GetServerClientCount(); i++)
	{
		...
		if (cl && ((C_BasePlayer*)cl)->IsActive() && ((C_BasePlayer*)cl)->IsConnected())// && !((CBaseEntity*)cl)->IsFakeClient())
		{
			C_BasePlayer* pEnt = (C_BasePlayer*)ServerClientToEntity(cl);
			if (pEnt)
			{
...
StartLagCompensationFn oStartLagCompensation = NULL; // value initialization might not be needed here.

bool BacktrackPlayer(C_BasePlayer* pEnt, float flTargetTime, std::deque<LagCompensationRecord*>* records)
{
	...
	while (curr < records->size())
	{
		...
		if (delta.LengthSquared() > (64.0f * 64.0f))
		{
			Vector absorigin = pEnt->GetAbsOriginServer();
		...
		(record->flSimulationTime < prevRecord->flSimulationTime))
	{
		...
		ang = Math::Lerp(frac, record->absangles, prevRecord->absangles);
		org = Math::Lerp(frac, record->absorigin, prevRecord->absorigin);
	...
	else
	{
		...
		eyeanglesfromrecord.y = Math::ClampYaw(Math::ClampYaw(record->absangles.y) + record->body_yaw);
...
void __fastcall hkStartLagCompensation(void* thisptr, int edx, void* player, int lagCompensationType, const Vector& weaponPos, const QAngle& weaponAngles, float weaponRange)
{
	C_BasePlayer* const ent = (C_BasePlayer*)player;
	...
	correct = std::clamp(correct, 0.f, 1.0f); //clamp to sv_maxunlag

	// correct tick send by player 
	float flTargetTime = TICKS_TO_TIME(cmd->tick_count) - m_fLerpTime;

	static CGlobalVarsBase** gppGlobals = nullptr;
	if (!gppGlobals)
	{
		gppGlobals = (CGlobalVarsBase**)Memory::Scan(XorStr("server.dll"), XorStr("8B 15 ?? ?? ?? ?? 66 0F 6E 50 08 F3 0F 10 62 10"));
		if (!gppGlobals)
#ifdef DEBUG_MODE
			DebugBreak();
#else
			return;
#endif
		gppGlobals = (CGlobalVarsBase**)*(DWORD*)((DWORD)gppGlobals + 2);
	}

	CGlobalVarsBase* gpGlobals = (CGlobalVarsBase*)*gppGlobals;
...
namespace Interfaces
{
	...
	bool Create( void* reserved ) {
		...
		static auto FrameUpdatePostEntityThink = Memory::Scan(XorStr("server.dll"), XorStr("55 8B EC 83 EC 3C A1 ? ? ? ? 53"));
		oLagCompFrameUpdatePostEntityThink = Hooked::HooksManager.CreateHook<decltype(oLagCompFrameUpdatePostEntityThink) >(&LagComp_FrameUpdatePostEntityThink, (void*)FrameUpdatePostEntityThink);

		static auto StartLagCompensation = Memory::Scan(XorStr("server.dll"), XorStr("55 8B EC 83 E4 F8 83 EC 48 56 8B F1 C7"));
