...
// possibly rework this as after looking at it im almost fairly certain that this doesn't determine the knife range; wouldn't be surprised since it's polakcode.
...
namespace Interfaces
{
	...
	void CKnifeBot::Main(Encrypted_t<CUserCmd> pCmd, bool* sendPacket) {
		...
		for (int i = 1; i <= Interfaces::m_pGlobalVars->maxClients; i++) {
			...
			for (auto& record : lag_data->m_History) {
				if (Interfaces::Ragebot::Get()->IsRecordValid(Target, &record)//Engine::LagCompensation::Get()->IsRecordOutOfBounds(record, 0.2f)
	...
	int CKnifeBot::DeterminateHitType(bool stabType, Vector eyeDelta) {
		...
		// then what the fuck is it still doing here alpha?
