...
namespace Engine
{
	...
	void C_AnimationData::Collect(C_CSPlayer* player) {
		...
		if (record->m_iChokeTicks > 0 && record->m_iChokeTicks < 16 && pThis->m_AnimationRecord.size() >= 2) {
			...
			//// big velocity fix ma nigga
			//float_t flWeight = (1.0f - record->m_serverAnimOverlays[11].m_flWeight) / 2.8571432;
			//if (flWeight > 0.0f) {
			//	float_t flSpeed = (flWeight + 0.55f) * player->GetMaxSpeed();
			//	if (flSpeed > 0.0f && record->m_vecVelocity.Length() > 0.0f)
			//		record->m_vecVelocity /= record->m_vecVelocity.Length() / flSpeed;
			//}
			//else if (!(record->m_fFlags & FL_ONGROUND)) {
			//	float_t flCurrentVelocityDirection = normalize_float(RAD2DEG(atan2(record->m_vecVelocity.x, record->m_vecVelocity.y)));
			//	float_t flPreviousVelocityDirection = normalize_float(RAD2DEG(atan2(previous_record->m_vecVelocity.x, previous_record->m_vecVelocity.y)));
			//	float_t flAverageVelocityDirection = DEG2RAD(normalize_float(flCurrentVelocityDirection + ((flCurrentVelocityDirection - flPreviousVelocityDirection) * 0.5f)));

			//	float_t flDirectionCos = cos(flAverageVelocityDirection);
			//	float_t flDirectionSin = sin(flAverageVelocityDirection);

			//	if (record->m_vecVelocity.Length2D() > 0.0f) {
			//		record->m_vecVelocity.x /= record->m_vecVelocity.Length2D();
			//		record->m_vecVelocity.y /= record->m_vecVelocity.Length2D();
			//	}

			//	if (flDirectionCos > 0.0f)
			//		record->m_vecVelocity.x *= flDirectionCos;

			//	if (flDirectionSin > 0.0f)
			//		record->m_vecVelocity.y *= flDirectionSin;

			//	record->m_vecVelocity.z -= g_Vars.sv_gravity->GetFloat() * TICKS_TO_TIME(record->m_iChokeTicks) * 0.5f;
			//}
		}

		// this looks weird because i redid the velocity fix
		...
		for (int i = 0; i < 13; i++) {
			if (record->m_serverAnimOverlays[i].m_nSequence == 26 && record->m_serverAnimOverlays[i].m_flWeight < 0.4f)
				stage1 = true;
			if (record->m_serverAnimOverlays[i].m_nSequence == 7 && record->m_serverAnimOverlays[i].m_flWeight > 0.001f)
				stage2 = true;
			if (record->m_serverAnimOverlays[i].m_nSequence == 2 && record->m_serverAnimOverlays[i].m_flWeight == 0)
				stage3 = true;
		}

		if (stage1 && stage2)
			// since weight from stage3 can be 0 aswell when crouching, 
			// we need this kind of check cause you can fakewalk while crouching
			// & thats why it's nested under stage1 and stage2.
			if (stage3 || (record->m_fFlags & FL_DUCKING))
				bFakewalking = true;
			else
				bFakewalking = false;
		else
		...
		record->m_bFakeWalking = bFakewalking;
