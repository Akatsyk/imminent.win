...
namespace Engine
{
		...
		try {
			...
			while (it != this->m_Weaponfire.end()) {
				...
				if (!did_hit) {
					...
					// reese, at some point ask geico if this is some anti-paste or some shit, what the fuck
	...
	void C_ShotInformation::EventCallback(IGameEvent* gameEvent, uint32_t hash) {
		...
		auto net_channel = Encrypted_t<INetChannelInfo>(Interfaces::m_pEngine->GetNetChannelInfo());
