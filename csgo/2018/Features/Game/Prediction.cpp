...
namespace Engine
{
	...
	void Prediction::PacketCorrection( uintptr_t cl_state ) {
		...
		if( *( int* )( uintptr_t( cl_state ) + 0x164 ) == *( int* )( uintptr_t( cl_state ) + 0x16C ) ) {
			...
			if( correct != predictionData->m_CorrectionData.begin( ) && correct != predictionData->m_CorrectionData.end( ) ) {
				// TODO; check and see if it would be better if we uncommented this & anything to do with m_flVelocityModifier after we redo the netvar compression.
	...
	void Prediction::KeepCommunication( bool* bSendPacket, int command_number ) {
		...
		// this should probably be uncommented lmao
	...
	// TODO; delete all of this as it is some bullshit and add back the PhysicsSimulate hook and redo the netvar compression so we are always targeting pristine data
	// after we redo that & the m_flVelocityModifier shit, start to rework the networking
	...
	void RestoreData::Apply( C_CSPlayer* player ) {
		...
		*( int* )( uintptr_t( player ) + hGroundEntity ) = this->m_hGroundEntity;
		...
		player->m_nTickBase( ) = this->m_nTickBase; // this and the one above was commented; could be antipaste, could not be
