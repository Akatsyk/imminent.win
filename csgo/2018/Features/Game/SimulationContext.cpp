...
// this is not used and/or initialized anywhere, so i have a feeling this is another one of geico's anti-pastes;
// or it had to do with the doubletap that was stripped from the source
...
void SimulationContext::ExtrapolatePlayer(float yaw) {
	...
	if (!g_GameRules || *(bool*)(*(uintptr_t*)g_GameRules + 0x20) || (this->flags & FL_FROZEN))
		return;
