...
void DrawWatermark() {
	...
	// background.
	Render::Engine::RectFilled(screen.x - size.m_width - 20, 10, size.m_width + 10, size.m_height + 2, Color(0, 0, 0, 220));
	Render::Engine::RectFilled(screen.x - size.m_width - 20, 10, 2, 16, g_Vars.menu.ascent.ToRegularColor());

	// text.
	Render::Engine::segoe.string(screen.x - 15, 10, { 255, 255, 255, 220 }, text, Render::Engine::ALIGN_RIGHT);
}
...
void CEsp::Indicators() {
	...
	if (g_Vars.esp.indicators) {
		...
		//	ind.text = std::to_string(rbot->min_damage_override_amount);
		...
		if (!indicators.empty()) {
			...
			for (size_t i{ }; i < indicators.size(); ++i) {
				...
				auto TextSize = Render::Engine::indicator.size(indicator.text);

				Render::Engine::indicator.string(12, Render::GetScreenSize().y - 60 - (30 * i), indicator.color, indicator.text);
			}
		}
	}
	...
	//			Render::Engine::indicator.string(12, Render::GetScreenSize().y - 60 - (30 * i), dumbass.clrOverride == Color::Black() ? Color(255, 170, 55) : dumbass.clrOverride, dumbass.str);
	//		}
	//	}
	//}
}
...
void CEsp::SpectatorList(bool window) {
	...
	if (!window) {
		...
		Color main = Color(0, 0, 0, 150 * alpha);

		// header
		Render::Engine::RectFilled(pos, this->m_SpecList.size, Color(0, 0, 0, 220 * alpha));
...
void CEsp::Keybinds() {
	...
	// header
	Render::Engine::RectFilled(pos, this->m_KeyBinds.size, Color(0, 0, 0, 220 * alpha));
	...
	for (int i = 0; i <= Interfaces::m_pEntList->GetHighestEntityIndex(); ++i) {
		...
		if (!_strcmpi(entity->GetClientClass()->m_pNetworkName, (XorStr("CPlantedC4")))) {
			...
			if (g_Vars.esp.draw_c4_bar && g_Vars.globals.bBombActive) {
				...
				if (timer_bomb > 0.f) {
					...
					if (WorldToScreen(origin, screen_origin)) {
						static const auto size = Vector2D(Render::Engine::pixel_reg.size(buf).m_width + 2, 4.f);
						...
						if (factor > 0.f) {
							...
							Render::Engine::pixel_reg.string(new_pos.x + (size.x * 0.5f), new_pos.y - 9, g_Vars.esp.c4_color.ToRegularColor().OverrideAlpha(180 * 0.87f), buf, Render::Engine::ALIGN_CENTER);
						}
					}
				}
			}
		...
		if (g_Vars.esp.draw_c4_bar) {
			if (entity->m_hOwnerEntity() == -1 &&
				...
				if (WorldToScreen(entity->GetAbsOrigin(), out)) {
					Render::Engine::pixel_reg.string(out.x + 2, out.y, g_Vars.esp.c4_color.ToRegularColor().OverrideAlpha(180 * 0.87f), XorStr("C4"));
				}
			}
		}
		...
		if ((g_Vars.esp.dropped_weapons || g_Vars.esp.dropped_weapons_ammo) && initial_alpha && !m_LocalPlayer->IsDead()) {
			...
			if (g_Vars.esp.dropped_weapons) {
				Render::Engine::pixel_reg.string(out.x + 2, out.y, g_Vars.esp.dropped_weapons_color.ToRegularColor().OverrideAlpha(static_cast<int>(initial_alpha)), name);
			}

			if (g_Vars.esp.dropped_weapons_ammo) {
				auto clip = weapon->m_iClip1();
				if (clip > 0) {
					const auto TextSize = Render::Engine::pixel_reg.size(name);
...
void CEsp::RenderNades(C_WeaponCSBaseGun* nade) {
	...
	Render::Engine::pixel_reg.string(size.x, size.y - 2, Color(255, 255, 255, 220), Name.c_str(), Render::Engine::ALIGN_CENTER);
}
...
void CEsp::DrawName(C_CSPlayer* player, BBox_t bbox, player_info_t player_info) {
	...
	Render::Engine::esp.string(bbox.x + bbox.w / 2, bbox.y - Render::Engine::esp.m_size.m_height - 1, clr, name, Render::Engine::ALIGN_CENTER);
}
