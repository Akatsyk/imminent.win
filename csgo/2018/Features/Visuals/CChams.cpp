...
namespace Interfaces
{
	...
	public:
		...
		virtual bool CreateMaterials() {
			...
			std::ofstream("csgo\\materials\\lean_shine.vmt") << R"#("VertexLitGeneric"
			{
			...
			m_matShiny = Interfaces::m_pMatSystem->FindMaterial(XorStr("lean_shine"), TEXTURE_GROUP_MODEL);
	...
	void CChams::DrawModel(void* ECX, IMatRenderContext* MatRenderContext, DrawModelState_t& DrawModelState, ModelRenderInfo_t& RenderInfo, matrix3x4_t* pBoneToWorld) {
		...
		if (client_class->m_ClassID == ClassId_t::CPredictedViewModel) {
			...
			if (g_Vars.esp.chams_hands_outline) {
				OverrideMaterial(false, MATERIAL_GLOW, g_Vars.esp.chams_hands_outline_color, std::clamp<float>((100.0f - g_Vars.esp.chams_hands_outline_value) * 0.2f, 1.f, 20.f), g_Vars.esp.chams_hands_outline_wireframe);
				Hooked::oDrawModelExecute(ECX, MatRenderContext, DrawModelState, RenderInfo, pBoneToWorld);
			}
	...
	bool CChams::GetBacktrackMatrix(C_CSPlayer* entity, matrix3x4_t* out) {
		...
		if (data.IsValid()) {
			...
			for (auto it = data->m_History.begin(); it != data->m_History.end(); ++it) {
				...
				if (!Interfaces::Ragebot::Get()->IsRecordValid(it->player, &*it) && it + 1 != data->m_History.end() && Interfaces::Ragebot::Get()->IsRecordValid(it->player, &*(it + 1)))
