...
void C_GameEvent::FireGameEvent(IGameEvent* pEvent) {
			...
			if (attacker_index != Interfaces::m_pEngine->GetLocalPlayer()) {
				if (enemy_index == local->EntIndex()) {
					if (g_Vars.esp.event_harm) {
						...
						msg << XorStr("Harmed by ") << enemy_info.szName << XorStr(" for ") << dmg_to_health << XorStr(" damage in the ") << HitgroupToString(hitgroup).data() << XorStr(".");
				...
				if (g_Vars.esp.event_dmg) {
					...
					msg << XorStr("Hit ") << enemy_info.szName << XorStr(" for ") << dmg_to_health << XorStr(" damage in the ") << HitgroupToString(hitgroup).data() << XorStr(".");
	...
	case hash_32_fnv1a_const("item_purchase"):
	{
		...
		std::stringstream msg;
		msg << info.szName << XorStr(" has bought a ") << pEvent->GetString(XorStr("weapon"));
		ILoggerEvent::Get()->PushEvent(msg.str(), FloatColor(255, 255, 255), true, XorStr(""));
	...
	case hash_32_fnv1a_const("bomb_begindefuse"):
	{
		...
		std::stringstream msg;
		if (has_defuse)
			msg << info.szName << XorStr(" started defusing with a kit.");
		else
			msg << info.szName << XorStr(" started defusing without a kit.");
	...
	case hash_32_fnv1a_const("bomb_abortdefuse"):
	{
		...
		std::stringstream msg;
		msg << info.szName << XorStr(" stopped defusing the bomb.");
	...
	case hash_32_fnv1a_const("bomb_pickup"):
	{
		...
		std::stringstream msg;
		msg << info.szName << XorStr(" picked up the bomb.");
	...
	case hash_32_fnv1a_const("bomb_beginplant"):
	{
		...
		std::stringstream msg;
		msg << info.szName << XorStr(" started planting the bomb.");
	...
	case hash_32_fnv1a_const("bomb_abortplant"):
	{
		...
		std::stringstream msg;
		msg << info.szName << XorStr(" stopped planting the bomb.");
