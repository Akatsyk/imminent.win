...
// both of these functions are not called anywhere; what the fuck did they do to the source?
...
namespace Interfaces
{
	...
	// TODO; mess with the autostop as it is shit
	...
	void C_Movement::AutoStrafe() {
		...
		// reese's jumpscout fix
		if (!(m_movement_data->m_pCmd->buttons & IN_JUMP) && m_movement_data->m_pLocal->m_vecVelocity().Length2D() < 30.f)
			return;
	...
	void ClampMovementSpeed(float speed) {
		...
		if (!LocalPlayer || LocalPlayer->IsDead() || !CurrentUserCmd.cmd)
			return;

		CurrentUserCmd.cmd->buttons |= IN_SPEED;

		float squirt = std::sqrtf((CurrentUserCmd.cmd->forwardmove * CurrentUserCmd.cmd->forwardmove) + (CurrentUserCmd.cmd->sidemove * CurrentUserCmd.cmd->sidemove));

		if (squirt > speed) {
			float squirt2 = std::sqrtf((CurrentUserCmd.cmd->forwardmove * CurrentUserCmd.cmd->forwardmove) + (CurrentUserCmd.cmd->sidemove * CurrentUserCmd.cmd->sidemove));

			float cock1 = CurrentUserCmd.cmd->forwardmove / squirt2;
			float cock2 = CurrentUserCmd.cmd->sidemove / squirt2;

			auto Velocity = LocalPlayer->m_vecVelocity().Length2D();

			if (final_speed + 1.0 <= Velocity) {
				CurrentUserCmd.cmd->forwardmove = 0;
				CurrentUserCmd.cmd->sidemove = 0;
			}
			else {
				CurrentUserCmd.cmd->forwardmove = cock1 * final_speed;
				CurrentUserCmd.cmd->sidemove = cock2 * final_speed;
			}
		}
	}

	// TODO; turn this into a fakewalk by making it compensate for lby too
	// preferably before we redo the aa system since it'll be even more useful then
	void C_Movement::SlowWalk(float speed, bool force_accurate) {
		...
		C_WeaponCSBaseGun* pCombatWeapon = (C_WeaponCSBaseGun*)LocalPlayer->m_hActiveWeapon().Get();
		if (!pCombatWeapon)
			return;

		auto info = pCombatWeapon->GetCSWeaponData();
		if (!info.IsValid())
			return;

		auto weaponInfo = info.Xor();
		...
		*m_movement_data->m_pSendPacket = Interfaces::m_pClientState->m_nChokedCommands() > g_Vars.misc.slow_walk_speed;
		...
		if (g_Vars.globals.m_bOldShot) {
			...
			float flMaxSpeed = LocalPlayer->m_bIsScoped() > 0 ? weaponInfo->m_flMaxSpeed2 : weaponInfo->m_flMaxSpeed;
			float flDesiredSpeed = (flMaxSpeed * 0.33000001);
