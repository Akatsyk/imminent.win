...
namespace Hooked
{
	...
	bool CreateMoveHandler(float ft, CUserCmd* _cmd, bool* bSendPacket, bool* bFinalTick) {
		...
		CurrentUserCmd.cmd_backup = *cmd.Xor();

		DWORD* fp;
		__asm mov fp, ebp
		CurrentUserCmd.FramePointer = fp;
		CurrentUserCmd.Reset(cmd.Xor());

		// this could cause issues.
		if (Interfaces::m_pClientState->m_nChokedCommands() < g_Vars.fakelag.iLagLimit)
		{
		...
		if (Interfaces::m_pEngine->GetNetChannelInfo()) {
			// make sure MessageHandler is active before continuation
			if (bMessageHandlerHooked && !Interfaces::m_pClientState->m_pNetChannel->m_MessageHandler)
				bMessageHandlerHooked = false;
			else if (!bMessageHandlerHooked && Interfaces::m_pClientState->m_pNetChannel->m_MessageHandler)
				bMessageHandlerHooked = true;

			if (bNetchanIsHooked && bMessageHandlerHooked) {
				if (!CurrentUserCmd.bFinalTick || !CurrentUserCmd.bOriginalSendPacket)
					...
					g_Vars.globals.Last_Server_ChokeCountSent = server_netchan.m_nChokedPackets;
					g_Vars.globals.Last_Server_InSequenceNrSent = server_netchan.m_nInSequenceNr;
					g_Vars.globals.Last_Server_OutSequenceNrSent = server_netchan.m_nOutSequenceNr;
		...
		if (*bSendPacket) {
			...
			g_Vars.globals.m_iPreviouslyChokedTicks = Interfaces::m_pClientState->m_nChokedCommands();
	...
	void __fastcall PredictionUpdate(void* prediction, void*, int startframe, bool validframe, int incoming_acknowledged, int outgoing_command) {
		...
		//CPrediction* cPrediction = (CPrediction*)prediction;

		//g_Vars.globals.bInActualPrediction = true;

		//// an engine paused check should go here!

		//bool bRecievedNewWorldUpdate = true;

		//float flTimeStamp = Interfaces::m_pEngine->GetLastTimeStamp();
		//bool bTimeStampChanged = cPrediction->m_flLastServerWorldTimeStamp != flTimeStamp;
		//cPrediction->m_flLastServerWorldTimeStamp = flTimeStamp;

		//g_Vars.cl_pred_optimize->nFlags &= ~FCVAR_HIDDEN;
		//g_Vars.cl_pred_optimize->SetValue(0);

		//// still starting at same frame, so make sure we don't do extra prediction ,etc.
		//if ((cPrediction->m_nPreviousStartFrame == startframe) && g_Vars.cl_pred_optimize->GetBool() && g_Vars.cl_predict->GetInt() && bTimeStampChanged)
		//{
		//	bRecievedNewWorldUpdate = false;
		//}

		//C_BasePlayer* LocalEntity = (C_BasePlayer*)Interfaces::m_pEntList->GetClientEntity(Interfaces::m_pEngine->GetLocalPlayer());

		//cPrediction->m_nPreviousStartFrame = startframe;

		//// Save off current timer values, etc.
		//cPrediction->m_SavedVars = *Interfaces::m_pGlobalVars.Xor();

		//typedef void(__thiscall* _UpdateFn)(CPrediction*, int, bool, bool, int, int);
		//cPrediction->_Update_New(0, bRecievedNewWorldUpdate, validframe, incoming_acknowledged, outgoing_command);

		//// Restore current timer values, etc.
		//*Interfaces::m_pGlobalVars.Xor() = cPrediction->m_SavedVars;
		...
		//	
		//g_Vars.globals.bInActualPrediction = false;
	}
}
// vader.tech invite https://discord.gg/GV6JNW3nze