...
void UpdateCustomNetChannels()
{
	if (bNetchanIsHooked)
	{
		...
		if (time >= flNextTime)
		{
			const float commandInterval = Interfaces::m_pGlobalVars->interval_per_tick;//1.0f / cl_cmdrate->GetFloat();
			const double maxDelta = (double)fminf(Interfaces::m_pGlobalVars->interval_per_tick, commandInterval);
			const float delta = (float)std::clamp(time - flNextTime, 0.0, maxDelta);
		...
		{
			FlowUpdate(&server_netchan, FLOW_INCOMING, g_Vars.globals.Last_Packet_Size);
			...
			FlowNewPacket(&server_netchan, FLOW_INCOMING, server_netchan.m_nInSequenceNr, server_netchan.m_nOutSequenceNrAck, g_Vars.globals.Last_ChokeCountSent, server_netchan.m_PacketDrop, g_Vars.globals.Last_Packet_Size);
		}
	}
}

// rebuilt senddatagram to fix issues that cause the client to go out of sync with the server while fakelagging.
