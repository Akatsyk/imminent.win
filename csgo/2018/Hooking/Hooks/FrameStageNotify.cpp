...
namespace Hooked
{
	...
	void __fastcall FrameStageNotify( void* ecx, void* edx, ClientFrameStage_t stage ) {
		...
		if (!Interfaces::m_pEngine->GetNetChannelInfo() && bNetchanIsHooked)
			bNetchanIsHooked = false;
		else if (Interfaces::m_pEngine->GetNetChannelInfo() && !bNetchanIsHooked) {
			// reset our netchannel info before saying that netchannel is hooked.
		...
		// possibly throw this in an else.
		if (bNetchanIsHooked)
